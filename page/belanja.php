...
<div class="row">
    <div class="col-lg-12">
      ...
      <option value="<?=date('Y');?>" <? if($tahuns==date('Y')) echo "selected='selected'"; ?>>
        <?=date('Y');?>
      </option>
      <option value="<?=date('Y')-1;?>" <? if($tahuns==date('Y')-1) echo "selected='selected'"; ?>>
        <?=date('Y')-1;?>
      </option>
      <option value="<?=date('Y')-2;?>" <? if($tahuns==date('Y')-2) echo "selected='selected'"; ?>>
        <?=date('Y')-2;?>
      </option>
    </select></td>
                            ...
                            <div class="table-responsive">
                                <table width="100%" cellpadding="4px" cellspacing="0" class="tb" id="myTable1" style="font-size:11px" border="1px">
                                    ...
                                    $sql = "SELECT * from t_belanja where Status='ON' and PaguAnggaran!='0'  " . $search . "   order by TahunAnggaran DESC";}
                                    else{
                                        $sql = "SELECT * from t_belanja where Status='ON' and KdPptk='".$_SESSION['KDUNIT']."'  and PaguAnggaran!='0' " . $search . "   order by TahunAnggaran DESC";                                      }
                                    $NO = 0;
                                    $pager = new PS_Pagination($connect, $sql, 25, 5, "tgl_kunjungan=" . $tgl_kunjungan . "&ruang=" . $ruang . "&nama=" . $nama . "&thn=" . $thn . "&norm=" . $norm, "index.php?link=BELANJA&");
                                    ...
                                    while ($data = mysql_fetch_array($rs)) { ?>
                                        <tr <? echo "class =";
                                         ...
                                         <!--   <td align="center"><?php echo $data['KdBelanja']; ?></td>-->
                                            ...
                                            <td align="right"><?php $sisax = $data['PaguAnggaran'] - $data['RealisasiAnggaran']; echo number_format($sisax, 2, ',', '.'); ?></td>
											...
											<a href="./index.php?link=<?= $_GET['link']; ?>&verif=1&id=<?= $data['Id']; ?>"> <button type="button" name="tom" class="btn btn-outline-success sm-btn" data-toggle="tooltip" data-placement="top" title="Proses" ><i class="fa fa-angle-double-right"></i></button></a> </td>
                                            <!-- <td align="center">
...
<? if (!empty($_GET['verif'])) { ?>
                ...
                <div class="row">
                    ...
                    <div class="col">
                        <div class="input-group mb-3">
                            ...
                            <input type="text" name="pptk" class="form-control" value="<?php $sisa = $jms['PaguAnggaran'] - $jms['RealisasiAnggaran']; echo number_format($sisa, 2, ',', '.'); ?>" disabled>
                        </div>
                    </div>
        ...
        <div class="modalx-content" style="background:white;border-top:solid 6px #0a9f92">
		 ...
		 where Id ='$id'";
            ...
            <table width="100%">
                <tr style="color:#fff">
                    <td width="90%" style="color:36bea6;font-size:20px">
                       <b> <h5 style="color:#000000;font-weight:bold"><i class="fa  fa-edit"></i>&nbsp;&nbsp;FORM EDIT REALISASI BELANJA</h5></b>
  ...
  <div class="form-group"><label for="exampleInputPassword1">Pagu Anggaran</label>
				...
				while ($listpoly = mysql_fetch_array($qrypoly)){
			 	?>
                              <option value="<? echo $listpoly['KodePptk'];?>" <?php if($listpoly['KodePptk'] == $_REQUEST['pptk']): echo 'selected="selected"'; endif; ?> ><? echo $listpoly['NamaPptk'];?></option>
                              <? 
			 	} 
			 ?>
                            </select>
