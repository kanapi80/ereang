        ...
        <div class="card" style="background: radial-gradient(circle, #1fe4f5 10%, #3fbafe  60%);">
                                ...
                                <form name="formsearch" method="get">
                                    <table width="100%" style="background-color:transparent">
                                        <tr>
                                            ...
                                            {echo $belanja; } ?>" placeholder="Nama Belanja ..." class="form-control" style="width:100%" /></td>
                                ...
                                <table width="100%" cellpadding="4px" cellspacing="0" class="tb" id="myTable1" style="font-size:11px" border="1px">
                                    ...
                                    <tr align="center">
                                       ...
                                       <!-- <th width="7%" rowspan="2" align="center">KODE BELANJA </th>-->
                                        ...
                                        <th width="21%" rowspan="2">PPTK</th>
                                        <!--  <th>KET</th>-->
                                        <th width="7%" rowspan="2">AKSI</th>
                                        <!--   <th>AKSI</th>-->
                                    </tr>
                                    <tr align="center">
                                        <th width="10%">PAGU </th>
                                    ...
                                    $sql = "SELECT * from t_belanja where Status='ON'  " . $search . " order by KdBelanja ASC";}
                                    else{
                                        $sql = "SELECT * from t_belanja where Status='ON' and KdPptk='".$_SESSION['KDUNIT']."'  " . $search . " order by KdBelanja ASC";  
                                    }
                                    $NO = 0;
                                    $pager = new PS_Pagination($connect, $sql, 15, 5, "tgl_kunjungan=" . $tgl_kunjungan . "&ruang=" . $ruang . "&nama=" . $nama . "&norm=" . $norm, "index.php?link=BELANJA&");
                                    ...
                                    while ($data = mysql_fetch_array($rs)) { ?>
                                        <tr <? echo "class =";
                                         ...
                                         <!--   <td align="center"><?php echo $data['KdBelanja']; ?></td>-->
                                            ...
                                            <td align="center"><? echo number_format($data['RealisasiAnggaran'], 2, ',', '.'); ?></td>
                                            <td align="center"><?php $sisax = $data['PaguAnggaran'] - $data['RealisasiAnggaran']; echo number_format($sisax, 2, ',', '.'); ?></td>
                                            <td align="center"><?php echo $data['TahunAnggaran']; ?></td>
                                            <td align="left"><?= $data['NamaPptk']; ?></td>
                                            <!--  <td ><?php echo $data['KET']; ?></td>-->
                                            <td align="center"><a href="./index.php?link=<?= $_GET['link']; ?>&verif=1&id=<?= $data['Id']; ?>"> <button type="button" name="tom" class="btn btn-outline-success sm-btn"><i class="fa fa-edit" style="color:success;"></i></button></a> </td>
                                            <!-- <td align="center">
            ...
            <a href='./jasa/edit_input.php?link=<?= $_GET['link']; ?>&idx=<?= $data['IDXDAFTAR'] ?>&nosep=<?= $data['NOSEP'] ?>&bulan=<?= $_GET['bulan'] ?>&tahun=<?= $_GET['tahun'] ?>&nip=<?= $data['NIP_JKN']; ?>&name=<?= $_SESSION['NIP'] ?>' onClick="javascript:return confirm('Anda yakin ingin mengedit Input Hasil Klaim dengan No.SEP : <? echo $data['NOSEP']; ?>')" class="btn btn-outline-success"><i class="fa fa-edit" style="color:#20d708"></i>&nbsp;&nbsp;Edit</a><? } ?> </td>-->
                                    ...
                                    <tr style="background:#36bea6;color:#fff;font-weight:bold">
                                        ...
                                        <td align="center"><?php $sisaxx = $jum['JUMLAH'] - $jum['JumReal']; echo number_format($sisaxx, 2, ',', '.'); ?></td>
                                        <td align="center"></td>
                                        <td align="center">&nbsp;</td>
                                        <td align="left">&nbsp;</td>
...
<? if (!empty($_GET['verif'])) { ?>
    ...
    <div id="myModalx" class="modalx">
		...
		$userdata = mysql_fetch_assoc($get); 	*/
            ...
            $id = $_GET['idBel'];
            $jm    = "SELECT  * FROM t_billbelanja
		 where Id ='$id'";
            $get = mysql_query($jm) or die(mysql_error());
            $jms = mysql_fetch_assoc($get);
			  
			  $idx = $_GET['id'];
			 $p    = "SELECT PaguAnggaran FROM t_belanja
		 where Id ='$idx'";
            $get = mysql_query($p) or die(mysql_error());
            $pg = mysql_fetch_assoc($get);
            ?>
            <table width="100%">
                <tr style="color:#fff">
                    ...
                    <td width="10%"><a href="index.php?link=BELANJA&verif=1&id=<?=$idx;?>"><span class="closex">&times;</span></a></td>
                </tr>
            </table>
            <form name="simpan" id="simpan" method="post" action="./page/UpdateRealisasi.php">
                ...
                <div class="row">
                    ...
                    <div class="col">
                        <div class="input-group mb-3">
                            ...
                            <input type="text" class="form-control" value="<? echo number_format($pg['PaguAnggaran'], 2, ',', '.'); ?>" disabled="disabled">
 ...
 <option value="<?=$jms['BlnRealisasi'];?>"><?=$jms['BlnRealisasi'];?></option>
                ...
                <div class="row">
                    ...
                    <div class="col">
                        <div class="input-group mb-3">
                            ...
                            <input type="text" name="pptk" class="form-control" value="<?php $sisa = $pg['PaguAnggaran'] - $jms['RealisasiAnggaran']; echo number_format($sisa, 2, ',', '.'); ?>" disabled>
                        </div>
                    </div>
                    ...
                    <div class="col">
                        <div class="input-group mb-3">
                            ...
                            <input type="text" name="jumlah" id="jumlah"  onkeypress="return hanyaAngka(event, false)" class="form-control"  value="<?=$jms['RealisasiAnggaran'];?>"> 
                        </div>
                    </div><input type="hidden" name="idbill" id="idbill"  onkeypress="return hanyaAngka(event, false)" class="form-control" placeholder="Input Realisasi..." value="<?=$jms['Id'];?>"><input type="hidden" name="idbel" id="idbel"  onkeypress="return hanyaAngka(event, false)" class="form-control" placeholder="Input Realisasi..." value="<?=$jms['IdBelanja'];?>">
                    <button class="btn m-t-120 m-r-10 btn-success" type="submit" style="width:100%;height:30px;"><i class="fa fa-save"></i>&nbsp;&nbsp;UPDATE</button>
                    <br />
            </form>
       ...
       <th>AKSI</th>-->
                    ...
                    while ($data = mysql_fetch_array($rs)) { ?>
                        <tr <? echo "class =";
                            ...
                            <td align="left"><?php echo $data['NamaUraian']; ?></td>
                             <td ><?php echo $data['KET']; ?></td>-->
                            <td align="center">  <button class="btn btn-success btn-sm" type="button" data-toggle="tooltip" data-placement="top" title="Edit" ><i class="fa fa-edit"></i></button>
                    ...
                    <tr style="font-weight:bold;background:#36bea6;color:#FFFFFF" class="progress-bar-striped">
                        <?
